...
const CardMember: React.FC<CardProps> = ({ title, price, description }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-2xl text-green-600 font-bold mb-3">{price}</p>
      <p className="text-gray-600 text-sm leading-relaxed">{description}</p>
      <a href="https://member.dabingfit.com/#/">
        <button className="mt-4 w-full bg-sec text-white py-2 px-4 rounded-lg hover:bg-third transition-colors duration-300">
          Pilih Paket
        </button>
      </a>
    </div>
